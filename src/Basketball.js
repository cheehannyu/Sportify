...
function Basketball() {
 const [games, setGames] = useState([]);
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "basketballGames"), (snapshot) => {
  ...
  const handleCreateGame = async (newGame) => {
    ...
    await addDoc(collection(db, "basketballGames"), {
  ...
  const handleJoinGame = async (gameId) => {
    if (userGamesCount >= 3) {
      alert('You can only be in a maximum of 3 basketball games at once.');
      return;
    }
    const gameRef = doc(db, "basketballGames", gameId);
  ...
  const handleTelegramSubmit = async (telegramId) => {
    ...
    const gameRef = doc(db, "basketballGames", selectedGameId);
    const gameSnap = await getDoc(gameRef);
    if (gameSnap.exists()) {
      const game = gameSnap.data();
  ...
  return (
    <div className="basketball-container">
      <h2>Find or create basketball matches in your area</h2>
      ...
      <p className="user-stats">You are currently in {userGamesCount}/3 basketball games</p>
      <button 
