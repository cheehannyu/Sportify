...
function Football() {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "footballGames"), (snapshot) => {
  ...
  const handleCreateGame = async (newGame) => {
    ...
    await addDoc(collection(db, "footballGames"), {
  ...
  const handleJoinGame = async (gameId) => {
    if (userGamesCount >= 3) {
      alert('You can only be in a maximum of 3 football games at once.');
      return;
    }
    const gameRef = doc(db, "footballGames", gameId);
  ...
  const handleTelegramSubmit = async (telegramId) => {
    ...
    const gameRef = doc(db, "footballGames", selectedGameId);
    const gameSnap = await getDoc(gameRef);
    if (gameSnap.exists()) {
      const game = gameSnap.data();
  ...
  const handleDeleteGame = async (gameId) => {
    ...
    await deleteDoc(doc(db, "footballGames", gameId));
  };

  // Add game to conqueredGames and remove from footballGames
  const handleConqueredGame = async (game) => {
  if (!window.confirm("Are you sure you've conquered this game?")) return;

  try {
    // Pull the full football game document to get player info
    const gameRef = doc(db, "footballGames", game.id);
    const gameSnap = await getDoc(gameRef);
  ...
  return (
    <div className="football-container">
      <h2>Find or create football matches in your area</h2>
      ...
      <p className="user-stats">You are currently in {userGamesCount}/3 football games</p>
      <button 
