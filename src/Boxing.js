...
function Boxing() {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "boxingGames"), (snapshot) => {
  ...
  const handleCreateGame = async (newGame) => {
    ...
    await addDoc(collection(db, "boxingGames"), {
  ...
  const handleJoinGame = async (gameId) => {
    if (userGamesCount >= 3) {
      alert('You can only be in a maximum of 3 boxing games at once.');
      return;
    }
    const gameRef = doc(db, "boxingGames", gameId);
  ...
  const handleTelegramSubmit = async (telegramId) => {
    ...
    const gameRef = doc(db, "boxingGames", selectedGameId);
    const gameSnap = await getDoc(gameRef);
    if (gameSnap.exists()) {
      const game = gameSnap.data();
  ...
  const handleDeleteGame = async (gameId) => {
    ...
    await deleteDoc(doc(db, "boxingGames", gameId));
  };

  // Add game to conqueredGames and remove from boxingGames
  const handleConqueredGame = async (game) => {
  if (!window.confirm("Are you sure you've conquered this game?")) return;

  try {
    // Pull the full boxing game document to get player info
    const gameRef = doc(db, "boxingGames", game.id);
    const gameSnap = await getDoc(gameRef);
  ...
  return (
    <div className="boxing-container">
      <h2>Find or create boxing matches in your area</h2>
      ...
      <p className="user-stats">You are currently in {userGamesCount}/3 boxing games</p>
      <button 
